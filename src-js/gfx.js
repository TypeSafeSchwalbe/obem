...
function obem_texture_from_image(img) {
    obem.enforce_init();
    const gl = obem.gl;
...
function obem_texture_width(texture) {
    if(!(texture.tex instanceof WebGLTexture)) {
        throw new Error("The given texture is invalid!");
    }
    return BigInt(texture.width());
}

function obem_texture_height(texture) {
    if(!(texture.tex instanceof WebGLTexture)) {
        throw new Error("The given texture is invalid!");
    }
    return BigInt(texture.height());
}
